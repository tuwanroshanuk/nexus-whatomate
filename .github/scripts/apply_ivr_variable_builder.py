...
Path("internal/calling/ivr_variables_test.go").write_text(r'''package calling
...
# -----------------------------------------------------------------------------
# Authenticated HTTP schema discovery endpoint
# -----------------------------------------------------------------------------
replace_once(
...
replace_once(
    ...
    handler_code + '// ListIVRFlows returns all IVR flows for the organization\n',
)
...
# -----------------------------------------------------------------------------
# Reusable variable picker
# -----------------------------------------------------------------------------
Path("frontend/src/components/calling/IVRVariablePicker.vue").write_text(r'''<script setup lang="ts">
...
# -----------------------------------------------------------------------------
# Flow-aware upstream variable registry
# -----------------------------------------------------------------------------
replace_once(
...
replace_once(
    ...
    registry_code + '// Load flow data from server\n',
)
replace_once(
    "frontend/src/views/calling/IVRFlowEditorView.vue",
...
# -----------------------------------------------------------------------------
# Node properties: testing, schema discovery, visual TTS tags
# -----------------------------------------------------------------------------
replace_once(
...
replace_once(
    "frontend/src/components/calling/IVRNodeProperties.vue",
    '// Goto flow targets\n',
    properties_logic + '// Goto flow targets\n',
)

old_tts = '''        <TabsContent value="text" class="mt-2">
          <Textarea
...
new_tts = '''        <TabsContent value="text" class="mt-2 space-y-2">
          <Textarea
            :model-value="config.greeting_text || ''"
            @update:model-value="(v: string) => updateConfigEntries(v ? { greeting_text: v, audio_file: undefined } : { greeting_text: v })"
            placeholder="Enter text for TTS..."
...
replace_once("frontend/src/components/calling/IVRNodeProperties.vue", old_tts, new_tts)
...
http_store_block = '''      <div class="space-y-1.5">
...
http_discovery_block = http_store_block + '''
...
replace_once("frontend/src/components/calling/IVRNodeProperties.vue", http_store_block, http_discovery_block)

# The successful implementation commit should not retain the one-shot patcher.
Path(".github/workflows/apply-ivr-variable-builder.yml").unlink(missing_ok=True)
Path(".github/scripts/apply_ivr_variable_builder.py").unlink(missing_ok=True)
